...
#ユーザー入力
print("◆◆◆ Minecraft Addon Manifest Generator v1.5.0 ◆◆◆")
...
def behava_pack():
	...
	min_engines = input('min_engineのバージョン(デフォルトの場合はそのままエンターを押してください): ').replace('.',',').replace(' ',',')
	if min_engines == '':
...
def resources_pack():
	...
	#"header"部分の値変更
	load_manifest['header']['description'] = descript
	load_manifest['header']['name'] = 'resources'
	load_manifest['header']['uuid'] = uuid_1
